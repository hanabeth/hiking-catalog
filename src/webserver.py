from flask import Flask, request, redirect, flash, jsonify, url_for, render_template
...
# Configuration for application. This include binding the engine to the Base class.
app = Flask(__name__)
...
def gconnect():
	...
	if result['issued_to'] != CLIENT_ID:
		response = make_response(json.dumps("Token client ID doesn't match the app."), 401)
	...
	if stored_access_token is not None and gplus_id == stored_gplus_id:
		response = make_response(json.dumps('User is currently already connected.'), 200)
		response.headers['Content-Type'] = 'application/json'
		return response
...
####################################################################################
# Create User - takes in login_session and creates the new user in the database.
####################################################################################
...
def gdisconnect():
	...
	if result['status'] == '200':
		...
		response = make_response(json.dumps('User has been disconnected.'), 200)
		response.headers['Content-Type'] = 'application/json'
		return response
	else:
		# Invalid token was given.
		response = make_response(json.dumps('There was an error when trying to revoke this token.'), 400)
		response.headers['Content-Type'] = 'application/json'
		return response
...
# of this trail. The description will vary in variety and volume of information.
# ####################################################################################
@app.route('/locations/<int:location_id>/JSON/')
def showTrailsJSON():
	trails = session.query(HikingTrail).order_by(HikingTrail.id.desc)
	return jsonfiy(HikingTrail = [i.serialize for i in trails])

####################################################################################
# Route to GET specific hiking trail and display the description of this trail.
# The description will vary in variety and volume of information.
# ####################################################################################
@app.route('/locations/<int:location_id>/<int:trail_id>/JSON/')
def showTrailDetailsJSON(location_id, trail_id):
	Hiking_Trail = session.query(HikingTrail).filter_by(id = trail_id).one()
	return jsonfiy(Hiking_Trail = Hiking_Trail.serialize)
...
####################################################################################
# GET locations - this route displays a list of locations that are currently available
# (available meaning that they have already been added to the database previously).
# From here, users can click on the desired location to get the listed hiking trails
...
def showLocations():
	locations = session.query(Location).all()
	latestTrails = session.query(HikingTrail).order_by(HikingTrail.id.desc()).limit(5)
	if 'username' not in login_session:
		return render_template('publicCatalogMain.html', locations=locations, trails=latestTrails)
	else:
		return render_template('catalogMain.html', locations=locations, trails=latestTrails)
...
def editLocation(location_id):
	...
	if updatedLocation.user_id != login_session['user_id']:
		return "<script>function myFunction() {alert('You are not authorized to edit this locastion.');}</script><body onload='myFunction()''>"
	if request.method == 'POST':
...
####################################################################################
# Route that the user is directed to when they select the option of deleting a location.
# This will contain a DELETE method.
# ####################################################################################
...
####################################################################################
# When a user selects a location from the main page, they will be directed to a list of
# hiking trails that are in that location.
# TODO: check on what to do when there are multiple routes for a page/action.
# ####################################################################################
...
def showTrails(location_id):
	...
	trails = session.query(HikingTrail).filter_by(location_id=location_id).all()
	if 'username' not in login_session or location.user_id != login_session['user_id']:
		return render_template('publicTrails.html', trails=trails, location=location)
	else:
		return render_template('trails.html', trails=trails, location=location, creator=creator)


###########################################################################################
# This is the route that invokes a method to add a new hiking trail to a specific location.
###########################################################################################
...
def newTrail(location_id):
	if 'username' not in login_session:
		return redirect('/login')
	if request.method == 'POST':
		#TODO: login authorization of user.
		trail = HikingTrail(
...
####################################################################################
# This route will be used when a user clicks on a hiking trail to see more information. They
# will be redirected to a page that has the hiking trail's details, along with options to edit
...
def showTrailDetails(location_id, trail_id):
	...
	if 'username' not in login_session or trail.user_ != login_session['user_id']:
		return render_template('publicTrailDetails.html', location_id=location_id, trail=trail)
	else:
		return render_template('trailDetails.html', location_id=location_id, trail=trail)


####################################################################################
# This method will be invoked when a user chooses the option to edit a specific hiking trail.
# This will contain a PUT method.
# ####################################################################################
@app.route('/locations/<int:location_id>/<int:trail_id>/edit', methods=['GET', 'POST'])
def editTrail(trail_id, location_id):
	...
	if editTrail.user_id != login_session['user_id']:
		return "<script>function myFunction() {alert('You are not authorized to edit this hiking trail.');}</script><body onload='myFunction()''>"
	if request.method == 'POST':
		...
		session.commit()
		return redirect(url_for('showTrailDetails', location_id=location_id, trail_id=trail_id))
	else:
		return render_template('editTrail.html', location_id=location_id, trail_id=trail_id, trail=editTrail)


####################################################################################
# Route for when the user selects that they want to delete a specific hiking trail from the parent
# locaton.
# TODO: figure out if I need to leave that location in the database still, or only persevere a location
# when that location has values (hiking trails).
# ####################################################################################
...
def deleteTrail(location_id, trail_id):
	...
	if deleteTrail.user_id != login_session['user_id']:
		return "<script>function myFunction() {alert('You are not authorized to delete this trail.');}</script><body onload='myFunction()''>"
	if request.method == 'POST':
		session.delete(deleteTrail)
		session.commit()
		return redirect(url_for('showTrails', location_id=location_id))
	else:
		return render_template('deleteTrail.html', location_id=location_id, trail=deleteTrail)
